...
var stopsFile = fs.readFileSync("./google_transit/stops2.txt","utf8");
var stopTimesFile = fs.readFileSync("./google_transit/stop_timesPT1.txt","utf8");
var transfersFile = fs.readFileSync("./google_transit/transfers.txt","utf8");
...
function transfersParse(transfersFile){
    var arrayOfObjects = [];
    var arr = transfersFile.split("\n");
    var newObj;
    ...
    arr.forEach(function(stop){
        ...
        newObj.stop_id = stop[0];
        newObj.xfer_id = stop[1];
...
transfersList = transfersParse(transfersFile);
...
var TransfersModel = require('./server/models/transfers-model');

mongoose.connect('mongodb://localhost/subway');

var wipeDB = function () {

    var models = [StopsModel, StopTimesModel, TransfersModel];
...
var seedTransfers = function () {

    TransfersModel.create(transfersList, function (err) {
        if (err) {
            console.error(err);
        }
...
var seedTimes = function () {
    ...
    StopTimesModel.create(stopTimesList, function (err) {
        ...
        console.log('Stop Times seeded!');
        process.kill(0);
    });
};

mongoose.connection.once('open', function () {
    wipeDB().then(seedStops).then(seedTransfers).then(seedTimes);
});
